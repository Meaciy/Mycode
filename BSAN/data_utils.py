...
def build_tokenizer(fnames, max_length, data_file):
    ...
    if os.path.exists(data_file):
        ...
        tokenizer =pickle.dump(tokenizer, open(data_file, 'rb'), protocol=pickle.HIGHEST_PROTOCOL)
    else:
        tokenizer = Tokenizer.from_files(fnames=fnames, max_length=max_length, parse=parse)
        #pickle.dump(tokenizer, open(data_file, 'wb'))
        tokenizer =pickle.dump(tokenizer, open(data_file, 'wb'), protocol=pickle.HIGHEST_PROTOCOL)
    return tokenizer
...
def build_embedding_matrix(vocab, embed_dim, data_file):
    if os.path.exists(data_file):
        ...
        fname = './APARN/glove/glove.840B.300d.txt'
...
class ABSAGCNData(Dataset):
    def __init__(self, fname, tokenizer, opt):
        ...
        for obj in tqdm(parse(fname), total=len(parse(fname)), desc="Training examples"):
            ...
            if opt.parseamr:
                ori_adj = np.zeros((len(text_list), len(text_list)), dtype=float)
                ...
                syntree = syntree[0]
                if opt.parseadj:
                    ori_adj = softmax(headp)
                elif opt.parsehead:
                    head = [leaf.father for leaf in syntree.leaves()]
                    ori_adj = np.zeros((len(text_list) + 1, len(text_list) + 1), dtype=headp.dtype)
                    for i, j in enumerate(head):
                        ori_adj[i + 1][j] = 1
                else:
                    raise Exception("Wrong Parse Opt!")
            ...
            assert len(text_list) == ori_adj.shape[0] == ori_adj.shape[1], '{}-{}-{}'.format(len(text_list), text_list,
                                                                                             ori_adj.shape)
